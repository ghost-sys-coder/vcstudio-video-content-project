...
export async function listUsageLedgerEntries(input: {
  ...
  await Promise.all([
    ...
    scriptIds.length === 0
      ...
      : database
          .select({
            id: scriptGenerationRuns.id,
            requestedByUserId: scriptGenerationRuns.requestedByUserId,
            model: scriptGenerationRuns.model,
            providerRequestId: scriptGenerationRuns.providerRequestId,
            triggerRunId: scriptGenerationRuns.triggerRunId,
            inputTokens: scriptGenerationRuns.inputTokens,
            outputTokens: scriptGenerationRuns.outputTokens,
            estimatedCostCents: scriptGenerationRuns.estimatedCostCents,
          })
          .from(scriptGenerationRuns)
          .where(
            and(
              eq(scriptGenerationRuns.workspaceId, input.workspaceId),
              inArray(scriptGenerationRuns.id, scriptIds),
            ),
          )
          .then((rows) => {
            for (const row of rows)
    ...
    thumbnailIds.length === 0
      ...
      : database
          .select({
            ...
            requestedByUserId: thumbnailGenerations.requestedByUserId,
            model: thumbnailGenerations.model,
            providerRequestId: thumbnailGenerations.providerRequestId,
            triggerRunId: thumbnailGenerations.triggerRunId,
            estimatedCostCents: thumbnailGenerations.estimatedCostCents,
          })
          .from(thumbnailGenerations)
          .where(
            and(
              eq(thumbnailGenerations.workspaceId, input.workspaceId),
              inArray(thumbnailGenerations.id, thumbnailIds),
            ),
          )
          .then((rows) => {
            for (const row of rows)
