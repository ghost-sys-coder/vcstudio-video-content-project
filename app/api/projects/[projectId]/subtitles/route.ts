...
import type { SubtitleWorkspaceResponse } from "@/lib/subtitles/subtitle-view";
import { subtitleRouteParamsSchema } from "@/lib/schemas/subtitle";

type Params = { projectId: string };

function jsonResponse(
  body: SubtitleWorkspaceResponse,
  status = 200,
): NextResponse<SubtitleWorkspaceResponse> {
...
export async function GET(
  ...
  if (!parsedParams.success)
    return jsonResponse(
      { success: false, error: "The subtitle request is invalid." },
      400,
    );
  ...
  try {
    ...
    const data = await loadSubtitleWorkspace({
      workspaceId: workspaceContext.activeMembership.workspaceId,
      project,
    });
    ...
    return jsonResponse(
      { success: false, error: "The subtitle workspace could not be loaded." },
      500,
    );
  }
}
