...
export async function GET(request: Request) {
  ...
  // minted for one workspace cannot attach an account to another.
  if (
  ...
  try {
    ...
    const { tokens, account } = await provider.exchangeCode({
      ...
      // X requires PKCE, and its provider derives the code verifier from this
      // state — which is why it is passed on only here. Verified above before
      // being handed over, so a forged value never reaches the provider.
      state,
    });
    ...
    // Never surface the provider's raw error: it can carry internal hints, and
    // this lands in a URL.
